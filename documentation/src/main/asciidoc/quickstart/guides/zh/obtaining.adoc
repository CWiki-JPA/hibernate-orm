...
*  `lib/spatial/` 目录包含有 `hibernate-spatial` Jar和需要的依赖（这些依赖，不包含在 lib/required/ 和lib/jpa/ 中）
*  `lib/osgi/` 目录包含有 `hibernate-osgi`  Jar和需要的依赖（这些依赖，不包含在 lib/required/ 和lib/jpa/ 中）
